...
note: >
  ...
  v0.6 (Ayla MVP Recommendation Contract, owner Product Architecture). Не
...
> Recommendation Contract v0.6 (proposed)** (`05 Architecture/`, далее
...
> `INSUFFICIENT_CONTEXT`; OQ-REC-1/2/4 закрыты. Где контракт молчит
...
> Recommendation Contract v0.6 (proposed); «v0.1: …» — история.
...
| **направление** — Canonical NBA (`target`, `target_outcomes` — рабочие коды) в формулировке для человека; `family` до пилота не заполняется и не показывается (KB-F: CD §71 п.2) | обязателен | контракт §3, §33, §34; **B2, B3** |
...
| `requires_health_check` исполнения | показывается честно: `True`/`UNKNOWN` → запись уходит к человеку, не `BOOKING_ERROR` | контракт §23 (safety до рекомендации + медгейт на записи); `docs/OPEN_DECISIONS.md` §98 |
...
- **`linkage_type`** — допустимые значения: `inline_accept`,
  ...
  spec или остаётся здесь): recon, действие 3, строка 10 матрицы. *контракт
...
| Не хватает контекста для ответственной рекомендации | **`INSUFFICIENT_CONTEXT`** (v0.2) | Не карточка. Если конкретный ответ человека продолжит сценарий — один вопрос (исход `CLARIFY`); иначе честное «пока не могу» с причиной (исход `NO_ACTION` или `BLOCKED`). Вопрос только если ответ меняет решение; повторный вопрос об известном не задаётся (KB-F: приведено к AYLA-DEC-0097) | контракт §30, §32; AYLA-DEC-0097; B5; доктрина 12.09 |
...
| Исполнение требует человека | `HEALTH_CHECK_*` (не `BOOKING_ERROR`) | На C05 честно: направление рекомендовано, запись по этой услуге создаётся через оператора | контракт §23 (safety до рекомендации + медгейт на записи); §98; Final Reconciliation §7.2 S3 |
...
3. **OQ-REC-3 — в реестр DRF-1349.** TTL suppression после мягкого отказа
   (жёсткий — блокировка без `reconsider_after`, факт). контракт молчит; норма
...
- **Контракт:** Ayla MVP Recommendation Contract v0.6 (proposed) — §3–5,
...
- 2026-09-22 — **KB-F (DRF-2270, из PR #21, на отдельное утверждение).**
  Редакция v0.2 из PR #21 (строки 5.1–5.6, 5.8–5.13 и 5.9 таблицы KB-E)
  вынесена владельцем в отдельное утверждение (журнал главного окна CD §71
  п.1, вне git). Отличия от текста PR #21: ссылки «контракт v1.0» → «v0.6»
  (так перенесён контракт в KB-F); в §1.1 `family` снят с карточки — до
  пилота не заполняется (CD §71 п.2); строка `INSUFFICIENT_CONTEXT` §5
  приведена к исходам AYLA-DEC-0097 (вопрос = `CLARIFY`, иначе `NO_ACTION` /
  `BLOCKED` с причиной; «C03 = DecisionReadiness» снято); «два гейта» →
  «safety до рекомендации + медгейт на записи» (AYLA-DEC-0096). Статус —
  как в PR #21 (`review` / `candidate`). При отказе — возврат к v0.1.
- 2026-09-12 — **v0.2 PROPOSED (A4, awaiting owner approval).** Переписан
  ...
  реестр DRF-1349; §8 — критерии под C04/C05. Не изменено (контракт молчит):
