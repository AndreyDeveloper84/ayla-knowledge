...
**2026-09-21:** «отдельное Product/Privacy owner decision» для proactive-уведомлений по opt-in (отчёты, вода, возврат к плану) принято — `AYLA-DEC-0087`. В этой части запись заменена; требование user-initiated для рекомендаций услуг остаётся в силе. Текст записи выше не менялся.
...
## Партия регистрации 2026-09-21 — пакет «Цель → План → Дневник → Диетолог» (DRF-2259)

Пятнадцать записей ниже (`AYLA-DEC-0087`…`AYLA-DEC-0101`):
- двенадцать решений пакета брифа `OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY` ред. 2, принятого владельцем целиком 2026-09-21 (K-1, G-1, P-1, P-2, P-3, D-1, DY-1, N-1, N-2, DT-1, W-1, M-1);
- три ранее данных решения владельца, на которые пакет ссылается как на основание: Plan Lite (18.09), таблица «цель → план» (19.09), «Забудь всё» стирает дневник (21.09).

Формулировки двенадцати решений пакета в поле `Decision` — **дословно из брифа**, как принял владелец («в точных формулировках брифа»).

**Номера.** Выбраны как следующие свободные по правилу «номер проверяется по обоим реестрам»: максимальный занятый на 2026-09-21 — `AYLA-DEC-0086`; `AYLA-DEC-0087`+ не встречаются ни в KB, ни в `ai-bot-platform@dev`, ни в рабочих документах Ayla. **Присвоение номеров подтверждает владелец в PR этой партии** (его разрешение 2026-09-21, п. 3).

**Единственный источник истины.** По решению владельца 2026-09-21 («Полные записи решений вести только в OWNER_DECISION_REGISTER как единственном источнике истины. В Ayla Decision Log добавлять по одной короткой индексной строке…») полные записи ведутся **только здесь**. В `02 Strategy/Ayla Decision Log.md` — по одной индексной строке на решение, без текста. Это не воспроизводит дефект KB-001.

**Регистрация и канонизация.** Владелец разрешил каноническое письмо по этим решениям (2026-09-21, п. 2 и п. 4). Запретов вида `Canonical writing: FORBIDDEN` партия не несёт. Поправки документов KB — отдельными PR (DRF-2260…2263).

**«ТЗ» и «инв. N»** в записях — техническое задание владельца на программу `docs/PROMPT-ORCHESTRATOR-GOAL.md` (рабочее дерево Ayla, вне git) и номера его инвариантов (§7).

**Нумерации не смешивать.** §§ журнала главного окна (`docs/CURRENT_DECISIONS_2026-09-16.md`), §§ реестра бота (`ai-bot-platform/docs/OPEN_DECISIONS.md`) и `AYLA-DEC` — три независимые нумерации. Ссылки на первую даны явно.

### AYLA-DEC-0087 — Goal необязательна; состав продукта; волна поправок KB; частичная замена Reset Roadmap (K-1)

```text
ID:             AYLA-DEC-0087
Date:           2026-09-21
Question:       Как привести KB к решениям владельца 16–21.09 (цель, план,
                дневник, вода, proactive, аллергии), если approved MVP Reset
                Roadmap и approved MVP User Journey противоречат друг другу
                (OD-AUDIT-004)?
Options:        (а) одна волна поправок, частичная замена Reset Roadmap новой
                версией; (б) полная замена Reset Roadmap; (в) не трогать KB до
                пилота. Выбран (а).
Decision:       K-1 — вариант (а). Цель в продукте необязательна: ни дневник,
                ни вода, ни вопрос, ни запись, ни явно выбранное действие не
                требуют цели. План (Plan Lite), дневник питания, ориентиры,
                вода, фуд-сканер, Диетолог-отчёты и proactive-уведомления по
                opt-in входят в продукт. Разрешаю одну волну поправок KB по
                перечню брифа, не меняя схему статусов KB: Reset Roadmap
                выпускается версией 1.1 с разделом “Заменённые разделы” — §2
                “Основной путь”, §5 Sprint 1–4 и §7 “Минимальные новые
                сущности” заменены новыми DEC и не действуют, остальные
                разделы действуют; в Canon Index — строка v1.1 CANONICAL
                (Supersedes v1.0, с перечнем заменённых разделов) и строка
                v1.0 SUPERSEDED. MVP User Journey → v1.3; UX-OD-005 заменяется
                новым UX-решением по CD §55; MVP Scope → v0.5; DEC-0046
                заменяется новым DEC о proactive. OD-AUDIT-004 закрыть этим
                решением. Номера DEC присваиваются при записи и подтверждаются
                мной в PR KB.
Rationale:      Все решения по контуру приняты после снимка KB 2026-09-08; KB
                запрещала то, что владелец велел делать. Частичная замена
                новой версией документа не требует нового статуса в
                .knowledge/schema.yaml (прецедент — Product Essence v1.1 →
                v1.2 в CANON_INDEX).
Scope:          Необязательность Goal во всех сценариях; состав продукта
                первого релиза; перечень поправок KB брифа (Итог 3). Proactive
                здесь — уведомления по opt-in (отчёты, вода, возврат к плану),
                не проактивные рекомендации услуг.
Affected docs:  02 Strategy/Ayla MVP Reset Roadmap.md (→ v1.1: §2, §5 Sprint
                1–4, §7 заменены); 01 Product/User Journeys/Ayla MVP User
                Journey Specification.md (→ v1.3); 01 Product/UX
                MVP/decisions/ux-owner-decisions.md (UX-OD-005 → UX-OD-006);
                02 Strategy/Ayla MVP Scope and Release Contract.md (→ v0.5);
                00 Foundation/Canon Governance/CANON_INDEX.md; 06 Safety and
                Governance/Consent Scope Registry.md (proactive).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67. Основания в журнале главного окна: §15.1 (Goal OPTIONAL, 16.09), §63 (состав, 21.09).

**Отношение к AYLA-DEC-0046.** DEC-0046 требует для любых proactive «отдельного Product/Privacy owner decision». Настоящее решение и есть такое решение для proactive-уведомлений по opt-in; в части proactive DEC-0046 заменён. Требование user-initiated для рекомендаций услуг не меняется.

**OD-AUDIT-004** (Reset Roadmap ↔ MVP User Journey) закрыт этим решением.

### AYLA-DEC-0088 — Plan Lite без веса (регистрация решения 2026-09-18)

```text
ID:             AYLA-DEC-0088
Date:           2026-09-18
Question:       Как цель должна достигаться, если нет плана?
Options:        (а) оставить ярлык+фильтр до внешнего пилота; (б) Plan Lite
                без веса — 1–3 обязательства-действия (записаться на услугу /
                дневник N дней / вода), только adherence, без наблюдений тела;
                (в) полный Personal Plan после юристов. Выбран (б).
Decision:       «делай (б) Plan Lite без веса, заводи лист». План из 1–3
                действий трёх типов (book_service, log_food, log_water);
                прогресс — только «N из M действий», без процентов результата;
                ни одного наблюдения тела (вес, ProgressObservation) на пути;
                нового scope согласия нет.
Rationale:      Цель без плана была ярлыком и фильтром каталога; вариант (б)
                не упирается в хранение веса (ждёт Privacy/Legal).
Scope:          Plan Lite в каталоге и клиентах. Не открывает
                DesiredOutcome/ProgressObservation (см. AYLA-DEC-0094).
Affected docs:  02 Strategy/Ayla MVP Reset Roadmap.md §5 Sprint 2 (заменён);
                05 Architecture/Ayla Personal Plan Contract.md (новый).
Status:         DECIDED
```

Source: `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §49 (владелец, 2026-09-18, дословно «делай (б) Plan Lite без веса, заводи лист»). Регистрируется в KB по AYLA-DEC-0087 (K-1: «§5 Sprint 1–4 → DEC Plan Lite (CD §49)»). Runtime-лист DRF-2101.

### AYLA-DEC-0089 — Таблица «цель → план по умолчанию» (регистрация решения 2026-09-19)

```text
ID:             AYLA-DEC-0089
Date:           2026-09-19
Question:       Как формируется план для цели: подбирает ли Ayla действия под
                цель, ведёт ли питание, как решает, как достигнуть цели?
Options:        Три уровня: A — курируемые шаблоны «цель → план»; B — питание
                в плане; C — NBA. Выбрано: A и B — да, A4 (еженедельный
                возврат к плану) — да, C — после пилота.
Decision:       «утверждаю таблицу, B да, A4 да — заводи». Таблица «goal_key →
                услуга (1/нед или 1/2 нед) · дневник N дн/нед · вода N
                раз/день · текст “почему”» — данные (шаблоны в каталоге,
                правка в админке), не код; Ayla предлагает, человек правит и
                подтверждает. Связь цели с целью питания — связать, не слить;
                прогресс — только «N из M».
Rationale:      Ayla не решала, как достигнуть цели; план составлял только
                человек.
Scope:          Шаблоны плана по goal_key. Частоты таблицы — PLAN_CADENCE
                (AYLA-DEC-0093). «Уровень C после пилота» уточнён AYLA-
                DEC-0097: минимальный детерминированный выбор уровнем C не
                считается.
Affected docs:  03 AI System/Contracts/planning-rules-registry.yaml
                (PLAN_CADENCE); 05 Architecture/Ayla Personal Plan Contract.md
                (новый).
Status:         DECIDED
```

Source: `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §51 (владелец, 2026-09-19). Регистрируется по AYLA-DEC-0087. Runtime-листы DRF-2122…2126.

### AYLA-DEC-0090 — Смена цели при активном плане (G-1)

```text
ID:             AYLA-DEC-0090
Date:           2026-09-21
Question:       Что происходит с активным планом, когда человек меняет цель?
Options:        (а) спросить: обновить план под новую цель или оставить
                текущий; (б) автоматически закрыть и предложить новый; (в)
                ничего не делать, только пометка. Выбран (а).
Decision:       G-1 — вариант (а). При смене цели активный план не закрывается
                молча. Ayla спрашивает: “Обновить план под новую цель” или
                “Оставить текущий”. При обновлении старый план получает статус
                superseded со ссылкой на новый, а новый создаётся только после
                явного подтверждения кнопкой; пока новый не подтверждён,
                старый остаётся активным. При “Оставить” план продолжает
                работать с пометкой, под какую цель он собран.
Rationale:      Инв. 7–9 ТЗ: система не меняет обязательства человека сама;
                предложение не равно подтверждению.
Scope:          PersonalPlan при смене ClientGoal; новый статус плана
                superseded и ссылка superseded_by.
Affected docs:  05 Architecture/Ayla Personal Plan Contract.md; 05
                Architecture/Ayla Goal and Desired Outcome Contract.md; 05
                Architecture/Ayla Domain Event Registry.md (plan.superseded).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0091 — План без цели (P-1)

```text
ID:             AYLA-DEC-0091
Date:           2026-09-21
Question:       Может ли план существовать без цели?
Options:        (а) да, только собранный человеком (source=manual); (б) да,
                плюс курируемый шаблон «без цели»; (в) нет. Выбран (а).
Decision:       P-1 — вариант (а). План может существовать без цели. Такой
                план собирает только сам человек: источник плана — manual,
                цель не указана, у каждого действия источник “выбрано
                человеком”. Ayla без цели план не предлагает. Отсутствие цели
                не блокирует дневник, воду и явно выбранные действия. Действие
                “записаться” в плане без цели считается только по категории,
                которую человек выбрал сам.
Rationale:      Goal необязательна (AYLA-DEC-0087); без цели Ayla ничего не
                выдумывает и не предлагает.
Scope:          Создание плана без ACTIVE-цели; источник действий
                user_selected.
Affected docs:  05 Architecture/Ayla Personal Plan Contract.md.
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0092 — Провенанс действия плана и одна актуальная версия (P-2)

```text
ID:             AYLA-DEC-0092
Date:           2026-09-21
Question:       Какой минимальный провенанс хранится у каждого действия плана?
Options:        (а) неизменяемые версии действия с пятью полями; (б)
                изменяемое действие плюс журнал изменений; (в) только поле
                source. Выбран (а).
Decision:       P-2 — вариант (а). У каждого действия плана хранится: источник
                (шаблон / выбрано человеком / изменено человеком / специалист
                / принятое предложение Ayla), ссылка и версия источника, кто
                изменил, когда, причина из закрытого списка и ссылка на
                заменённое действие. Изменение действия создаёт новую версию,
                старая сохраняется; удаление — это версия со статусом
                “удалено”, а не стирание. В расчётах (выполнение “N из M”,
                выбор следующего шага, показ плана) участвует только одна
                актуальная версия действия; старые и удалённые версии остаются
                историей и в текущий расчёт не попадают.
Rationale:      Правка человека должна быть отличима от предложения Ayla (инв.
                25 ТЗ); одна строка — одно состояние, без второго источника
                истины.
Scope:          PlanAction и все расчёты по плану (adherence, Decision Engine,
                показ).
Affected docs:  05 Architecture/Ayla Personal Plan Contract.md; 03 AI
                System/Contracts/plan-action.schema.json (новая).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0093 — PLAN_CADENCE и запрет курсов из поведения (P-3)

```text
ID:             AYLA-DEC-0093
Date:           2026-09-21
Question:       Как частоты Plan Lite (таблица AYLA-DEC-0089) соотносятся с
                реестром правил планирования, где REPETITION =
                INTENTIONALLY_UNSUPPORTED?
Options:        (а) новый тип PLAN_CADENCE в реестре; (б) отдельный реестр
                частот; (в) оставить вне реестра. Выбран (а).
Decision:       P-3 — вариант (а). Частоты Plan Lite — это утверждённая мной
                регулярность привычек в плане, а не курс процедур. Заносятся в
                реестр правил планирования новым типом PLAN_CADENCE со
                статусом KNOWN, источник — моё решение §51 и версия шаблона;
                контракт планировочных ограничений поднимается до v1.1,
                перечень типов расширяется мной до 14. PLAN_CADENCE — только
                подтверждённая человеком организационная регулярность действия
                в его плане; она не доказывает медицинскую или
                косметологическую пользу, совместимость процедур, минимальный
                или максимальный интервал и обязательное число процедур,
                никогда не применяется к услуге как курс и не может
                приводиться как такое основание. Запрет выводить курс процедур
                из поведения пользователя (ADR-0012 N-04) утверждаю как
                отдельное решение; REPETITION, MIN_INTERVAL и MAX_INTERVAL для
                услуг остаются как есть.
Rationale:      Организационная регулярность привычки и курс процедур — разные
                сущности; запрет выдуманных курсов сохраняется и получает
                основание уровня решения вместо draft ADR-0012.
Scope:          03 AI System/Contracts/planning-rules-registry.yaml (перечень
                типов 13 → 14, subject_kind plan_template);
                PLANNING_CONSTRAINTS_CONTRACT v1.0 → v1.1 (ai-bot-platform
                docs/specs). REPETITION, MIN_INTERVAL, MAX_INTERVAL для
                canonical_service не меняются.
Affected docs:  03 AI System/Contracts/planning-rules-registry.yaml;
                scripts/validate_planning_rules.py; 05 Architecture/ADR-0012
                Dynamic User Model.md (N-04 — теперь основание AYLA-DEC-0093);
                05 Architecture/Ayla Personal Plan Contract.md.
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0094 — Desired Outcome не активируется в первом релизе (D-1)

```text
ID:             AYLA-DEC-0094
Date:           2026-09-21
Question:       Активировать ли Desired Outcome в первом релизе?
Options:        (а) не активировать, модели обездвижены; (б) активировать без
                веса; (в) удалить модели. Выбран (а).
Decision:       D-1 — вариант (а). Desired Outcome в первом релизе не
                активируется. Модели DesiredOutcome, PlanOutcomeLink и
                ProgressObservation остаются обездвиженными до утверждения
                таксономии целей и результатов; новый источник истины для
                “желаемого результата” не создаётся. Срок цели (target_date) и
                цель анкеты питания остаются единственными живыми носителями.
Rationale:      (б) создаёт второй источник истины рядом с
                ClientGoal.goal_text/target_date; (в) уничтожает работу,
                нужную после таксономии.
Scope:          DesiredOutcome, PlanOutcomeLink, ProgressObservation —
                писателей нет до утверждения Goal/Outcome Taxonomy.
Affected docs:  05 Architecture/Ayla Goal and Desired Outcome Contract.md; 05
                Architecture/Ayla Goal Outcome Semantic Model Working
                Design.md (заметка, статус не меняется).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0095 — Завершение дня дневника и исправление закрытого дня (DY-1)

```text
ID:             AYLA-DEC-0095
Date:           2026-09-21
Question:       Когда день дневника считается завершённым, что показывать до
                этого и что делать при исправлении закрытого дня?
Options:        (а) кнопка «Завершить день» плюс автозакрытие предыдущего дня
                в 04:00 по поясу человека; (б) только автозакрытие в полночь;
                (в) только кнопка. Выбран (а).
Decision:       DY-1 — вариант (а). Запись дневника принадлежит локальной
                календарной дате своего времени по часовому поясу человека
                (пояс — из профиля, иначе салона, иначе Москва; источник пояса
                записывается); запись после 00:00 относится к новой дате и во
                вчерашний день автоматически не переносится. День завершается
                кнопкой “Завершить день” или автоматически в 04:00 — это
                момент автозакрытия предыдущего дня, а не граница
                принадлежности записей. До завершения показываются только
                текущие числа без оценки “уложился / не уложился”; оценка дня,
                “в ориентире” у плана и серии дней считаются только по
                завершённым дням. Исправлять и удалять свои записи человек
                может всегда: день остаётся завершённым и помечается
                “исправлен”; за последние 7 суток пересчитываются оперативные
                отчёты и текущий прогресс; более старые исправления
                сохраняются в истории и учитываются в последующих агрегатах,
                но не пересчитывают старые отчёты. Уже отправленные сообщения
                не переотправляются, и повторные сообщения по старым дням не
                запускаются.
Rationale:      Оценка незавершённого дня нарушает инв. 14–15 ТЗ; исправление
                своих данных человеку не запрещается.
Scope:          Дневник питания и вода; оценка дня, «в ориентире» плана, серии
                дней; новая сущность «день дневника».
Affected docs:  05 Architecture/Ayla Diary and Water Contract.md; 03 AI
                System/Contracts/diary-day.schema.json (новая); 05
                Architecture/Ayla Domain Event Registry.md (diary.day.closed,
                diary.day.corrected).
Status:         DECIDED
```
...
### AYLA-DEC-0096 — Порядок safety и consent (N-1)

```text
ID:             AYLA-DEC-0096
Date:           2026-09-21
Question:       Что выполняется первым: проверка опасности сообщения или
                проверка согласия?
Options:        (а) четыре ступени: минимальный входной детектор → consent и
                context authorization → расширенная safety → функции; (б)
                буква Journey: consent первым для всего; (в) полная safety
                первой с чтением памяти. Выбран (а).
Decision:       N-1 — вариант (а). Первым выполняется минимальный входной
                детектор опасности — только по тексту текущего сообщения, без
                памяти и профиля; кризис и неотложка получают
                детерминированный ответ всегда, в том числе при согласии, не
                данном или отозванном, и при работе оператора. Затем —
                проверка согласий и того, какой контекст разрешено читать для
                этой цели; затем — расширенная оценка безопасности с
                разрешённым контекстом; затем — обычные функции. Порядок
                “consent/privacy → safety” в MVP User Journey относится к
                составлению рекомендаций из персонального контекста и
                уточняется поправкой v1.3.
Rationale:      Кризисная реплика должна получать ответ при любом состоянии
                согласия; память до согласия не читается.
Scope:          Входной путь каждого сообщения; уточняет область фразы
                «consent/privacy → safety» MVP User Journey §Recommendation
                and Proactivity Gates — она относится к составлению
                рекомендаций из персонального контекста.
Affected docs:  01 Product/User Journeys/Ayla MVP User Journey
                Specification.md (→ v1.3); 05 Architecture/Ayla Decision
                Policy Contract.md (новый).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0097 — Минимальный детерминированный Decision Engine до пилота (N-2)

```text
ID:             AYLA-DEC-0097
Date:           2026-09-21
Question:       Включать ли детерминированный выбор следующего шага до пилота,
                если AYLA-DEC-0089 относит уровень C (NBA) на после пилота?
Options:        (а) минимальный детерминированный выбор до пилота, сложный NBA
                после; (б) всё после пилота; (в) включить теневую политику как
                есть. Выбран (а).
Decision:       N-2 — вариант (а). До пилота включается минимальный
                детерминированный выбор следующего шага: безопасность → явное
                намерение человека (если оно есть, выполняется оно) →
                незавершённый сценарий (предложить, не навязывать) →
                подтверждённое действие на текущее время → следующее действие
                активного плана → предложение собрать план → ничего. Исходы
                разделены: ACT — есть допустимое полезное действие; CLARIFY —
                конкретный ответ человека способен продолжить сценарий,
                задаётся один вопрос; NO_ACTION — полезного действия или
                вопроса сейчас нет; BLOCKED — действие запрещено согласием,
                безопасностью, возрастом или выключателем, человеку
                объясняется, что недоступно; HANDOFF — нужен человек-оператор.
                Недостаточность данных не приравнивается к NO_ACTION. Каждый
                исход несёт исход, причину, ссылки на факты, происхождение,
                заблокированные функции, время расчёта и версию политики;
                модель только формулирует выбранное. Сложный NBA, скоринг и
                обучение — после пилота. Этим уточняется моё решение §51
                “уровень C после пилота”: минимальный выбор уровнем C не
                считается.
Rationale:      Без наблюдаемого исхода и причины не выполнимы инв. 24–25 ТЗ;
                минимальный выбор не является уровнем C.
Scope:          Выбор следующего шага там, где человек не выразил явного
                намерения; закрытый набор исходов ACT / CLARIFY / NO_ACTION /
                BLOCKED / HANDOFF.
Affected docs:  05 Architecture/Ayla Decision Policy Contract.md (новый); 03
                AI System/Contracts/decision-outcome.schema.json (новая); 05
                Architecture/Ayla MVP Recommendation Contract.md.
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0098 — Флаги здоровья не передаются во внешнюю LLM (DT-1)

```text
ID:             AYLA-DEC-0098
Date:           2026-09-21
Question:       Допустимо ли передавать беременность, кормление, диабет и
                другие флаги здоровья во внешнюю модель LLM-комментария?
Options:        (а) не передавать ни флаги, ни признак их наличия; при флаге
                LLM-комментарий не вызывается, локальный нейтральный текст;
                (б) передавать после отдельного согласия; (в) общий режим
                careful_mode во внешнюю модель — отвергнут, сам раскрывает
                наличие ограничения. Выбран (а).
Decision:       DT-1 — вариант (а). Беременность, кормление, диабет и любые
                другие флаги здоровья во внешнюю модель не передаются — ни
                сами флаги, ни производный признак их наличия. До отдельного
                согласия и контракта красной зоны для человека с любым флагом
                здоровья внешний LLM-комментарий не вызывается: каталог
                возвращает локальный детерминированный нейтральный текст без
                оценки и советов.
Rationale:      Специальные категории данных и трансграничная передача без
                согласия (согласие E — после пилота).
Scope:          Любой вызов внешней LLM из контуров питания и Диетолога.
Affected docs:  05 Architecture/Ayla Dietitian Capability Contract.md (новый);
                06 Safety and Governance/Consent Scope Registry.md (→ v1.5).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0099 — Размер стакана — настраиваемая UI-механика (W-1)

```text
ID:             AYLA-DEC-0099
Date:           2026-09-21
Question:       Нужен ли пользовательский размер стакана?
Options:        (а) 250 мл по умолчанию, настраивается человеком; (б)
                фиксированные 250 мл; (в) спросить при первой записи. Выбран
                (а).
Decision:       W-1 — вариант (а). Стакан 250 мл — стартовое значение
                интерфейса; человек может задать свой размер. Вода всегда
                хранится в миллилитрах; размер стакана — удобство интерфейса,
                а не норма, и нормой нигде не называется. Новый размер
                применяется только к будущим записям; прошлые записи воды не
                пересчитываются. Не блокер пилота.
Rationale:      Хранение в миллилитрах не меняется; стакан — удобство
                интерфейса, не норма.
Scope:          Запись воды во всех каналах; только будущие записи.
Affected docs:  05 Architecture/Ayla Diary and Water Contract.md.
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

### AYLA-DEC-0100 — Аллергии — красная зона; возраст — самодекларация; фильтр с UNKNOWN (M-1)

```text
ID:             AYLA-DEC-0100
Date:           2026-09-21
Question:       Как Ayla хранит и использует аллергии, если approved MDC §12 и
                DEC-0081 запрещают активацию yellow/red, а CSR §11.1 относит
                аллергии к yellow?
Options:        Зона: (а) красная по решению §63 полностью; (б) жёлтая по
                букве CSR §11.1; (в) отложить. Возраст: (i) дата рождения один
                раз, самодекларация; (ii) галочка «мне есть 18»; (iii) внешняя
                проверка. Выбраны (а) и (i).
Decision:       M-1 — варианты (а) и (i). Аллергии хранятся в красной зоне.
                Фраза распознаётся только как кандидат; Ayla показывает
                распознанную формулировку и спрашивает отдельное явное
                согласие. До сохранения человек один раз называет дату
                рождения — это самодекларация, а не юридическая проверка, и
                статусы так и называются (age_self_declared,
                adult_eligibility_asserted); без даты или при возрасте младше
                18 запись не создаётся. Полная дата рождения хранится в одном
                месте — в профиле каталога, наружу отдаются только статус и
                возраст в годах; это единственный источник возраста в
                продукте. Аллергии читают только рекомендации (фильтр состава)
                и сканер (предупреждение), в текст для модели они не попадают.
                При неизвестном или неполном составе результат фильтра —
                “неизвестно”, а не “безопасно”; никаких гарантий безопасности
                по одному отсутствию найденного аллергена. Каждый доступ и
                каждый отказ в доступе журналируются (memory.allergy.accessed,
                memory.allergy.access_denied); “Забудь аллергии” и “Забудь
                всё” удаляют запись, экспорт её отдаёт. Разрешаю поправки KB:
                Memory Domain Contract §12, Consent Scope Registry (аллергии —
                red, закрыть CSR-OD-4 в этой части, правило о
                несовершеннолетних), Data Inventory, export/forget, реестр
                событий, глоссарий.
Rationale:      Решение владельца §63: аллергии включены, через красную зону и
                проверку возраста; защита несовершеннолетних не ослабляется.
Scope:          MemoryEntry kind=allergy (red); дата рождения как единственный
                источник возраста; фильтр рекомендаций и предупреждение
                сканера. Для kind=allergy условия MDC §12 объявляются
                выполненными; прочие yellow/red — без изменений.
Affected docs:  05 Architecture/Ayla Memory Domain Contract.md (→ v1.1 §12);
                06 Safety and Governance/Consent Scope Registry.md (→ v1.5
                §11.1, §11.4; CSR-OD-4 частично закрыт); 06 Safety and
                Governance/Data Inventory Matrix.md; 05 Architecture/AMD-001
                C5 Pilot Personal Context Export-Forget Contract.md; 05
                Architecture/Ayla Domain Event Registry.md; 03 AI
                System/Contracts/allergy-filter-result.schema.json, age-
                status.schema.json (новые).
Status:         DECIDED
```

Source: `docs/Принимаю весь пакет решений из OWNER_DEC.md` (владелец, 2026-09-21 13:08): «Принимаю весь пакет решений … редакция 2 … — в точных формулировках брифа»; бриф — `docs/OWNER_DECISION_BRIEF_GOAL_PLAN_DIARY.md`, редакция 2 (рабочее дерево Ayla, вне git); журнал — `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §67.

**Отдельное подтверждение владельца (тот же файл, дословно):** «аллергии относятся к красной зоне. Перед сохранением Ayla показывает, как поняла аллергию, получает отдельное явное согласие и запрашивает дату рождения. Дата рождения является самодекларацией, а не юридической проверкой возраста. Без даты рождения или при возрасте младше 18 лет аллергия не сохраняется. Полная дата рождения хранится в одном месте; другим модулям передаются только возрастной статус и возраст в полных годах. Аллергии могут локально использовать только фильтр рекомендаций и сканер. Во внешнюю LLM они не передаются. Если состав неизвестен или неполон, результат — UNKNOWN, без утверждений о безопасности. Доступы и отказы журналируются. Аллергии входят в экспорт и удаляются командами «Забудь аллергии» и «Забудь всё».»

### AYLA-DEC-0101 — «Забудь всё» стирает дневник питания (регистрация решения 2026-09-21)

```text
ID:             AYLA-DEC-0101
Date:           2026-09-21
Question:       Стирает ли «Забудь всё» дневник питания (записи еды, вода,
                сохранённые блюда, фото еды), если по тексту команды его нет в
                списке того, что остаётся?
Options:        (а) стирать по тексту команды; (б) оставить и дописать в
                текст. Выбран (а).
Decision:       «а — стирать дневник вместе со всем». «Забудь всё» (оба пути:
                бот и приложение) стирает FoodLog, WaterEntry, SavedMeal,
                FoodScan вместе с фотографиями в той же транзакции «всё или
                ничего», что и цели, план и профиль питания. Текст команды не
                меняется: код приводится к тексту. Выгрузка по ст. 14 несёт
                дневник.
Rationale:      Команда обещает забыть всё; дневник — персональные данные,
                внесённые человеком.
Scope:          «Забудь всё» и экспорт персональных данных.
Affected docs:  05 Architecture/AMD-001 C5 Pilot Personal Context Export-
                Forget Contract.md (снимается исключение wellness history в
                части дневника); 06 Safety and Governance/Data Inventory
                Matrix.md; 05 Architecture/Ayla Diary and Water Contract.md.
Status:         DECIDED
```

Source: `docs/CURRENT_DECISIONS_2026-09-16.md` (журнал главного окна, вне git) §66 (владелец, 2026-09-21, вопрос 60, дословно «а — стирать дневник вместе со всем»). Runtime-лист DRF-2214.
