...
- Wrong version pin: "Journey v0.3" → **"Journey v1.1"** (Ayla MVP User Journey Specification v1.1, draft / proposed / candidate).
...
- User Journey references (UJS этапы 10–13, N6/N7 on the same line) → revalidate against the revised UJ Specification v1.1 (draft / proposed / candidate) after D1.
...
- **Acceptance criteria**: every Mobile App surface referenced by UJS v1.1 (post-revision) has a navigation entry; no MAX-only framing.
...
| 12 | D3 | I-03, I-04, I-05, I-24 | Update CDM refs; fix Recommendation Contract (memory-first framing, Journey pin v1.1, §16 channels); fix ADR-0013 refs + broken path; fix Transaction State Model broken recommendation-contract path and revalidate UJ refs; Architecture Review | Step 8 (D1 gate) | `05 Architecture/Ayla Core Domain Model Specification.md`, `05 Architecture/Ayla MVP Recommendation Contract.md`, `05 Architecture/ADR-0013 Recommendation Snapshot.md`, `05 Architecture/Ayla Transaction State Model.md` | `canon: align architecture contracts with MVP v0.3` | 0 errors; broken-path sweep clean | Architecture Review | Step 13 |
...
| Architecture Review | I-03, I-04, I-05, I-24 | v0.3 refs only; Journey pin v1.1; memory-first framing removed; no broken paths (incl. Transaction State Model) | canonical contradiction; broken reference | APPROVED / NEEDS_WORK | D5 |
...
| 0.1 | 2026-08-03 | D1 Journey reference normalization: current Journey target normalized from stale "v1.0, canonical" to actual "v1.1, draft / proposed / candidate" (§4 I-01 row; §5 I-04/I-24 audit; §10 Step 12 D3; §13 Architecture Review gate); "Journey v0.3" → "Journey v1.0" wrong-pin correction targets updated to "Journey v1.1" throughout. Historical entries (rows above, including the 2026-07-31 "Journey v0.3" → "Journey v1.0" repair record) preserved unchanged as accurate history of that pass. No version bump; no wave order, inventory identity, owner-decision, or gate-semantics change. |
