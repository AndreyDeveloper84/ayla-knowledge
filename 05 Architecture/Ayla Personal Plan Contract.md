...
- Состояние «пауза» **не вводится** (см. «Открытые вопросы»).
...
- Поле в этой редакции **не вводится** и в схеме отсутствует — открытый вопрос 2.
...
Все команды выполняет только человек (или специалист — открытый вопрос 4) через кнопку или экран. **LLM ни одной команды не вызывает** (инв. ТЗ 7, 8).
...
| I-8 | План без цели — только ручной (`source = manual`), все действия `source = user_selected`. Ayla без цели план не предлагает и не создаёт по своей инициативе. `book_service` такого плана считается только по категории, выбранной человеком | AYLA-DEC-0091; решение владельца 21.09 (CD §71 п.3) |
...
| R-4 | `book_service` без цели — по категории, выбранной человеком | без категорий цели засчитывается **любая** бронь (`cat:wellness/fact_providers.py:70-80`); носителя категории нет (`cat:wellness/models.py:489-490`) | открытый вопрос 2 → лист P-1 |
...
- Состояние плана «пауза» (решением не утверждено).
- Напоминания и расписание действий; время действия — открытый вопрос 1.
...
1. **Время действий плана.**
   - CD §63 (21.09): «теперь не откладываем: время действий плана».
   - Формы нет ни в одном решении: поле, повтор по дням недели, пояс, связь с напоминаниями.
   - От ответа зависит ступень AYLA-DEC-0097 «подтверждённое действие на текущее время» для действий плана.
   - Поля в схеме нет. Нужно решение владельца.
2. **Носитель категории `book_service`** в плане без цели (AYLA-DEC-0091).
   - В коде категории у действия нет (`cat:wellness/models.py:489-490`).
   - Варианты: поле версии действия (например, ссылка на категорию каталога), влияющее на `source_ref`, или отдельная связь.
   - Имя и форма поля не выдуманы и в схему не введены.
3. **Пауза плана.** ТЗ §6 и Slice 2 называют «паузу», решения владельца её не утверждают. Не вводится.
4. **Специалист** (`source = specialist`, `changed_by = specialist`, `reason_code = specialist_change`).
   - Пути записи специалистом нет.
   - Не определены: права специалиста на план клиента, согласие клиента, формат `source_ref`, подтверждает ли клиент правку специалиста.
...
| AYLA-DEC-0097 | Входы и выходы; I-12; R-15; Открытый вопрос 1; A-12, A-17 |
