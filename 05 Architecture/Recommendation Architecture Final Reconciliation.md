...
> текст ниже больше не дословная копия.** Узел перенесён по решению
> владельца 21.09 (журнал главного окна CD §71 п.1, вне git: полезные
> изменения PR #21 — отдельно, на самостоятельное утверждение). Документ
> датирован 12.09; решения владельца AYLA-DEC-0087…0101 (21.09) и CD §71
> п.2–3 **старше** его по правилу §1.2 (более поздняя дата на уровне 1).
> Места, которые им противоречат, в тексте сняты или помечены
> «〔KB-F: …〕» и не действуют:
>
> - семейства `ADDRESS / SUPPORT / RECOVER / OBSERVE` «provisional для
>   Controlled Pilot» (B9; слой 8, §5, §6.2, §7, §8) — до пилота не
>   включаются, `family` = `null` (CD §71 п.2);
> - `DesiredOutcome` refs, `desired_outcomes[]`, `semantic_readiness ⇒ ≥ 1`
>   (слой 3, §4 конфликт 3, §5) — Desired Outcome не активируется,
>   используются рабочие коды Outcomes (AYLA-DEC-0094);
> - «C03 = адаптивный DecisionReadiness flow» (слой 4, §3 п.7, §4
>   конфликт C, §6.2) — до пилота DecisionReadiness только в тени; вопрос
>   человеку выбирает минимальный детерминированный выбор (AYLA-DEC-0097);
> - вариант исполнения `plan` для любого NBA (слой 11) — без цели Ayla план
>   не предлагает (AYLA-DEC-0091; CD §71 п.3);
> - «сквозной» safety-слой и «два safety-гейта» (слой 6, инвариант (з)) —
>   порядок четырёх ступеней AYLA-DEC-0096: минимальный детектор по тексту
>   сообщения идёт раньше слоя 1, слой 6 — ступень 3;
> - LLM формулирует WHY (слой 9, инвариант (д)) — аллергии и флаги
>   здоровья в текст модели не попадают (AYLA-DEC-0100, 0098).
>
> Ссылки «контракт v1.0» ниже читаются как
> [[Ayla MVP Recommendation Contract]] v0.6 (KB-F). При отказе владельца узел удаляется.
...
| 3 | Semantic Resolution (Goal / Desired Outcomes) | Какое изменение человек хочет и зачем? | Intent Output + user expression + allowed context | `SemanticResolutionResult`: `goal?` 0..1, `outcomes[]` 0..N (рабочие коды), `resolution_status` 〔KB-F: `desired_outcomes[]` и `semantic_readiness` сняты — AYLA-DEC-0094〕; Goal не синтезируется; **Goal optional — рекомендация не требует создания Goal (B5)** | PB (таксономия — Владелец) | WD §11.2 (OD-SR-1…6, OD-DC-1); контракт §27–§29 → A1 | **B5**, A1 |
| 4 | Adaptive Clarification (C03) | Что уточнить, чтобы не спрашивать лишнего? | `resolution_status`, InterpretationGroup, required context facts | `SKIP / CONFIRM_ONE / CHOOSE_MANY / ASK_CONTEXT`; ≤1 вопрос за ход; после ответа re-resolution; вопрос человеку — исход `CLARIFY` AYLA-DEC-0097; **анкета `area → feeling → goal` перестаёт быть обязательным входом (B5)** 〔KB-F: «C03 = адаптивный DecisionReadiness flow» снято — AYLA-DEC-0097〕 | PB | WD §11.1, §11.3; Intent Model §Confidence and Clarification (≤2 подхода); доктрина 12.09 «повторный вопрос об известном → движок не задаёт» | **B5**, доктрина |
...
| 11 | **Execution Mapping (C05, HOW)** | Как это можно реализовать? | принятый NBA | execution options: `SERVICE_PATH | self-care | observe | plan` 〔KB-F: `plan` — только по AYLA-DEC-0091: без цели Ayla план не предлагает, собрать его может только сам человек〕; Capability ← Need/Outcome | PB по онтологии; онтология — Каталог/Владелец | контракт §34; канон v1.1 Decision 11 §14 (`Need → Capability → CanonicalService → TenantOffer`) | draft / B1 |
...
- (д) LLM участвует на слоях 1–3 (извлечение), 9 (формулировка WHY), 10 (рендер) 〔KB-F: аллергии и флаги здоровья в текст модели не попадают — AYLA-DEC-0100, 0098〕; на слоях 6–8, 12–13, 15 — не участвует в решении (канон v1.1 §21; DRE §17.1 «Ни одного вызова LLM»);
...
- (з) safety не один гейт: conversational safety state (слой 6, до Recommendation, B6) и service-level health gate на записи (слой 15, §98) 〔KB-F: плюс ступень 1 AYLA-DEC-0096 до слоя 1; «ровно два» снято〕; кандидат с `requires_health_check ∈ {True, UNKNOWN}` может быть рекомендован как направление, исполнение уходит к человеку — это раскрывается в WHY/C05 честно;
...
| 7 | Не задавать вопрос без material reason | «Next question is allowed only if its answer can change admissibility, ranking, or required execution parameters» — канон v1.1 §8; BOT-003-Q3 APPROVED; **B5** 〔KB-F: «C03 = адаптивный DRE flow» снято — AYLA-DEC-0097〕 | К + В |
...
Recommendation:                          # immutable decision record — канон (OQ-R1; канон v1.1 §10.1); хранится по retention policy — В (B13)
  ...
  decision_subject:                      # ЧТО = Canonical NBA — В (B2)
    ...
    target_outcomes: []                  # рабочие коды Outcomes — KB-F: не DesiredOutcome refs (AYLA-DEC-0094)
    family:                              # KB-F: до пилота null (CD §71 п.2); B9 «provisional» снято
...
Минимум для пилота (без него запись не считается Recommendation): `recommendation_id`, `recommendation_set_id`, `recommendation_role`, `decision_subject.direction_code` + `target_outcomes` 〔KB-F: `family` снят — CD §71 п.2〕, `result_status`, `readiness_state`, `reason_codes`, `evidence_refs`, `explanation.displayable`, `safety_evaluation_ref`, версии политик, `catalog_mapping_version`, `actionable_until`.

Изменения относительно черновика §5: `expires_at` → `actionable_until` (B13; retention вынесен в политику); `family` из «отложено §24» → provisional (B9) 〔KB-F: не действует — CD §71 п.2〕; `alternative_recommendation_ids` — правило показа B4; `context_snapshot_ref` — один из трёх снимков B10; блок событий — B8; `decision_subject` = NBA закрыт B2; PendingBookingIntent — D4.
...
- **A1 к Recommendation Contract** — дельты `05 Architecture/Ayla Goal Outcome Semantic Model Working Design.md` §20: D-1…D-5 (Goal optional, Semantic Resolution вместо Goal/Outcome Resolution — B5), D-6 (C01 Hybrid First Contact, C02 = outcome resolution с Adaptive Clarification, C03 = adaptive DecisionReadiness flow — B5 〔KB-F: C03-часть снята — AYLA-DEC-0097〕), D-7 (`action_type` → C05), D-9, D-10 (`SemanticResolutionResult` upstream, Context Sufficiency = decision-level). D-8 (families) — закрыт B9 как provisional `ADDRESS / SUPPORT / RECOVER / OBSERVE` 〔KB-F: не действует: до пилота семейства не включаются — CD §71 п.2〕.
...
Не в этом пакете (без изменения статуса): терминология `DesiredOutcome` (OQ-GO-1), постоянство NBA taxonomy после пилота (〔KB-F: до пилота семейств нет — CD §71 п.2〕; OQ-R11/OQ-GO-9), числовые окна attribution (Measurement Framework), TTL suppression (OQ-REC-3), Killer PRD §5 (OQ-R12), строки slice 4, 5, 6, 8 (после первой волны).
