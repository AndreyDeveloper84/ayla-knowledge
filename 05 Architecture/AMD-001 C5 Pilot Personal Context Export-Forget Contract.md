...
- `contract_version` — AMD-001 version string (e.g., `"0.8"`);
...
| 67 | AMD001-W6-065 | AMD001 v0.8 §5.8 status-read schema validation | Status-read response conforms to operation-status-read schema | Any operation exists | Perform status read | 200 OK | Response validates against operation-status-read/1.0 schema; additionalProperties rejected | N/A | none | N/A | privacy.status_read | yes | Status read contract is machine-enforceable | Schema validation script + HTTP response | NOT_IMPLEMENTED | NOT_EXECUTED | SPECIFIED |
...
- The architectural decision content, version, status and decision status were
  not changed.
...
**Конец документа — AMD-001 v0.8.1 (Draft, pending technical re-review)**