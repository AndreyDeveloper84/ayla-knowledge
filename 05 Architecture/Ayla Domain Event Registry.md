...
> **Статус:** Draft v0.7 — proposed (версия во frontmatter и здесь
> совпадает; см. Change Log v0.7). Создан по мандату AYLA-DEC-0025 п. 1
...
> **v0.7 (KB-F, DRF-2270) — из ayla-knowledge PR #21, на отдельное
> утверждение владельца** (журнал главного окна CD §71 п.1, вне git):
> `recommendation.accepted` / `.declined` → `deprecated`; новые
> `recommendation.explanation_requested`, `.alternative_requested`,
> `.engaged`, `booking_intent.created` — `proposed` (§6.5). При отказе
> владельца эти правки снимаются, реестр возвращается к v0.6.
...
> **v0.7 (KB-F, из PR #21; B8; канон v1.1 §17.3, §17.5; контракт v0.6
> §15, §17) — PROPOSED, на отдельное утверждение.** Таксономия событий
> Recommendation — **одна**, product event backbone канона v1.1 §17.3.
> `recommendation.accepted` и `recommendation.declined` — **deprecated**
> (B8: «ENGAGED доказывает взаимодействие, `booking_intent.created` —
> переход к исполнению; generic accepted интерпретируется сильнее
> доказанного»; явный отказ — reaction `REJECTED` interaction-слоя, не
> отдельное domain-событие). Добавлены `proposed`:
...
> Payload — минимальный по контракту §15/§17 и канону v1.1 §16.4; поля,
> которых источники не называют, помечены TBD.
...
event_version: 1
registration_status: deprecated        # v0.7 PROPOSED (KB-F) — снято B8 (контракт v0.6 §15, §17)
semantic_status: defined
semantic_class: domain_fact
publication_scope: cross_context
authoritative_owner: Channel Delivery / Interaction
...
notes: >
  ...
  v0.7 (KB-F, B8): событие снято — generic acceptance интерпретируется
  сильнее доказанного; взаимодействие = recommendation.engaged, переход к
  исполнению = booking_intent.created. Не публикуется после утверждения
  владельцем; consumers переключаются на engaged/booking_intent.created.
```
...
notes: >
  ...
  v0.7 (KB-F, B8): событие снято; явный отказ фиксируется как reaction
  REJECTED (канон v1.1 §10.3) в interaction-слое, не как отдельное
  domain-событие.
```
...
event_version: 1
registration_status: proposed          # v0.7 (KB-F) — B8; регистрация после утверждения владельцем
semantic_status: defined
semantic_class: domain_fact
publication_scope: cross_context
authoritative_owner: Channel Delivery / Interaction
...
event_version: 1
registration_status: proposed          # v0.7 (KB-F) — B8
semantic_status: defined
semantic_class: domain_fact
publication_scope: cross_context
authoritative_owner: Channel Delivery / Interaction
...
event_version: 1
registration_status: proposed          # v0.7 (KB-F) — B8, D4
semantic_status: defined
semantic_class: domain_fact
publication_scope: cross_context
...
trigger: >
  ...
  (контракт v0.6 §17; канон v1.1 §16.1, §16.4). Несёт именно ту
...
payload:
  ...
  # execution_option_ref → Execution Mapping Snapshot (контракт v0.6 §7): состав — TBD при регистрации
...
notes: >
  ...
  expired — канон v1.1 §17.3) в v0.7 не регистрируются: контракт v0.6
  их не описывает.
```
...
| `recommendation.*` — interaction (presented; v0.7 PROPOSED: explanation_requested, alternative_requested, engaged; accepted/declined — deprecated, B8) | Channel Delivery / Interaction | Channel Adapter (MAX) | active (v0.3, registered; v0.7 — proposed) |
| `booking_intent.created` (v0.7 PROPOSED, B8/D4) | Booking / Handoff | Ayla backend (PendingBookingIntent) | proposed (v0.7, KB-F) |
...
| `RecommendationAccepted` | Roadmap §6.4 | ~~`recommendation.accepted`~~ → `recommendation.engaged` (взаимодействие) / `booking_intent.created` (переход к исполнению) | re-mapped (v0.7 PROPOSED, B8) |
...
- **OQ-E1 (ЗАКРЫТ — v0.3).** MVP-состав `recommendation.*` определён
  ...
  **v0.7 (PROPOSED, KB-F, B8):** состав пересмотрен по контракту v0.6 §15 —
...
### v0.7 (2026-09-22) — KB-F, DRF-2270: таксономия Recommendation из PR #21 (B8), на отдельное утверждение
...
- **§6.5:** `recommendation.accepted`, `recommendation.declined` →
  ...
  §16.4 и D4, состав `execution_option_ref` — TBD). Записи — как в PR #21
  (там — «v0.6»; номер v0.6 занят KB-E), ссылки «контракт v1.0» → «v0.6».
- **§8, §12, §13 (OQ-E1):** таблица владельцев и migration mapping
  приведены к новому составу; `RecommendationAccepted` re-mapped.
- Не изменено: `recommendation.created / superseded / expired /
  ...
  `interaction.*`, `feedback.submitted`) — контракт их не описывает.
- Основание: решение владельца 21.09 (журнал главного окна CD §71 п.1, вне
  git) — полезные изменения PR #21 на самостоятельное утверждение; таблица
  — `docs/audits/2026-09-22-pr21-rest-kb-f.md`. Статус документа не
  изменён: draft / proposed. При отказе — возврат к v0.6.
