...
trigger: Услуга подтверждённо оказана (authoritative completion).
payload:
  required: [appointment_id]
...
notes: >
  semantic_status: incomplete (v0.2, ревью п. 3): не определено, кто
  вправе объявить услугу оказанной (отметка мастера, статус внешней
  booking system, спор, no-show, исправимость события, отличие от
  наступления времени) — предмет Appointment Contract (OQ-E3).
  Прохождение времени записи не является этим событием (MVP User
  Journey v0.3, этап 14). Обязательный кандидат для Stage 14; до
  Appointment Contract не считается полностью определённым.
```
...
На 2026-07-29 событий со статусом `pending-semantic-definition` в
реестре нет. Незакрытая семантика, остающаяся в реестре:
`appointment.completed` — `semantic_status: incomplete` до Appointment
Contract (OQ-E3); candidate mappings семейства `memory.*` (§6.4, §11).
...
| `appointment.*` | Appointment | Appointment Service (CAP-011) | active (`appointment.rescheduled` — registered v0.4, AYLA-DEC-0022) |
...
## 10. Delivery Semantics (proposal)
...
| `AppointmentCompleted` | Roadmap §6.4, CDM §11 | `appointment.completed` | candidate — semantic_status incomplete (OQ-E3) |
...
- **OQ-E3.** Точный payload и authoritative completion для Stage 14
  (`appointment.completed` vs «время записи прошло»; кто вправе объявить
  услугу оказанной; спор, no-show, исправимость; связь с
  `OutcomeRecorded`).
