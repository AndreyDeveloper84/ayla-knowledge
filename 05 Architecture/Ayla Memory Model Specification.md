...
An **AI inference** is not an Observation. It is a hypothesis generated by a
model, which may arise from Observations but does not become an Observation
automatically. Like an Observation, an AI inference is not a user fact and may
become a `MemoryProposal` only after explicit user confirmation and a whitelist
check.
...
Not every `MemoryProposal` requires direct user confirmation. Some proposals
may be accepted through another canonically approved provenance or policy path.
...
| `ai_inferred` | AI model | Model hypothesis; not a fact until confirmed | YES | NO — explicit confirmation required per AYLA-DEC-0023 | YES | YES |
...
`USUALLY YES` indicates the canonical default for this conceptual category, but
the exact confirmation rule is `NOT_DEFINED` at the level of Wave 2A.
...
| **AI Inference** | A hypothesis generated by a model. | AI / model | NO | NO | NO | Explicit user confirmation + whitelist check → `MemoryProposal` → `MemoryEntry`. |
...
| Conversation Context | NO | NO | Conversation Domain | Session-scoped continuity context. |
