...
| Wave | Wave 1 - Foundation (locked); Wave 2A - Memory Semantics (locked); Wave 2B - Memory Identity (completed, pending review) |
| Active Canon | no |

This document contains the locked Wave 1 Foundation, the locked Wave 2A Memory Semantics, and the completed Wave 2B Memory Identity, pending targeted review.
...
An **AI inference** is not an Observation. It is a hypothesis generated by a model, which may arise from Observations but does not become an Observation automatically. Like an Observation, an AI inference is not a user fact. For an AI inference, the canonical path is explicit user confirmation, then a `MemoryProposal`, then the whitelist check before persistence.
...
For an AI inference, the canonical order is AI inference -> explicit user confirmation -> `MemoryProposal` -> whitelist check -> governed persistence boundary. Other proposals may be accepted through another canonically approved provenance or policy path. The existence of a proposal does not imply a particular confirmation mechanism.
...
For ai_inferred, the canonical order is AI inference -> explicit user confirmation -> `MemoryProposal` -> whitelist check -> governed persistence boundary.
