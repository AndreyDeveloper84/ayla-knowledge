...
### «Цель словами человека» — до пилота (решение владельца CD §73)

Решение владельца 22.09, журнал главного окна CD §73 (вне git), дословно «2 до пилота». Путь исполнения — DRF-2283.

1. **Слова человека — как есть.** Человек записывает цель своими словами, например «хочу −5 кг к лету». Ayla хранит её в `goal_text` дословно, с пометкой источника `user_stated`: это слова человека, а не вывод системы. Имя поля пометки определяет реализация DRF-2283, значение `user_stated` — из решения. Пересказ модели, подпись чипа и нормализованный текст этой пометки не получают (G-INV-4).
2. **Показ.** Карточка цели показывает слова человека дословно.
3. **Числа в словах — не цель системы.** Величина и срок внутри `goal_text` («−5 кг», «к лету») не извлекаются в поля. Они не становятся таргетом, ориентиром или сроком плана. Ayla не оценивает их реалистичность или безопасность: проверки темпа и исходов вида «ACCEPT / CAUTION» у цели нет. Цели в килограммах с проверкой темпа не строятся — CD §49, DRF-1331 в силе.
4. **Прогресс — никогда от веса.** Прогресс по цели не считается ни от веса, ни от других наблюдений тела. План строится из действий (таблица «цель → план», `AYLA-DEC-0089`), мера — только «N из M» ([[Ayla Personal Plan Contract]]). Desired Outcome остаётся обездвиженным (`AYLA-DEC-0094`, G-INV-9).
5. **Безопасность текста** — как у любого свободного текста цели: safety-вход до записи (G-INV-13; раздел «Безопасность»).
...
| G-INV-15 | «Цель словами человека»: свободный текст цели хранится дословно с пометкой источника `user_stated` и показывается дословно. Числа и сроки в тексте не извлекаются в таргет, ориентир или срок плана и не оцениваются. Прогресс по цели не считается от веса и наблюдений тела; мера — только «N из M» действий плана. | решение владельца 22.09, журнал главного окна CD §73 (вне git); CD §49; `AYLA-DEC-0088`, `AYLA-DEC-0094` | инв. 3, 4, 6 |
...
| «Цель словами человека» (G-INV-15, CD §73) | `goal_text` дословно с пометкой источника `user_stated`; показ дословно в карточке; числа не извлекаются и не оцениваются; прогресс не от веса | `goal_text` хранится дословно (`cat:goals/models.py:106-112`); поля пометки источника нет (есть только `source_channel` — канал, не источник слов); прогресса по цели нет | **DRF-2283** (ayla-anketa) |
...
- Цели в килограммах, `RequestedTarget`, проверка темпа и исходы вида «ACCEPT / CAUTION» для величины цели — не строятся (CD §49, CD §73, DRF-1331). Числа в словах человека остаются словами (G-INV-15).
...
| AC-15 | Цель «хочу −5 кг к лету», введённая человеком, сохранена дословно с пометкой источника `user_stated` и показана в карточке дословно. Ни одно поле таргета, ориентира или срока плана не заполнено из чисел текста; прогресс по цели от веса не показан; план — из действий, мера «N из M» (после DRF-2283). | E2E 2 |
...
- **Пометка источника `user_stated` (CD §73, DRF-2283):** аддитивное поле `ClientGoal`. Существующие строки со свободным `goal_text` — это ввод человека через `SelectGoal` или анкету, поэтому бэкфилл `user_stated` для них допустим. Строки только с `goal_key` (чип) пометку не получают.
...
| Решение владельца 22.09, журнал главного окна CD §73 (вне git) — «цель словами человека» до пилота | Сущности («Цель словами человека»); Инварианты G-INV-15; Граница API; Не входит; Миграции; AC-15 |
...
### v0.1 — решение владельца CD §73 вписано (DRF-2261)

- «Цель словами человека» до пилота (CD §73, 22.09, вне git): раздел в «Сущности», G-INV-15, строка «Граница API» (исполнение DRF-2283), «Не входит» (цели в кг и проверка темпа не строятся), «Миграции» (пометка `user_stated`), AC-15, трассировка.
